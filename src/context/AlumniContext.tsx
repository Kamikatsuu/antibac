...
export const AlumniProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const permissions = useMemo(() => {
    ...
    return {
      ...
      // Access employer dashboard: employer or admin/staff
      canAccessEmployerDashboard: role === 'employer' || ['admin', 'superadmin', 'staff'].includes(role || ''),
      ...
      // Post Jobs: admin/staff, OR verified employer with canPostJobs !== false, OR verified alumni
      canPostJobs: ['admin', 'superadmin', 'staff', 'registrar'].includes(role || '') ||
    ...
    (studentIdToCheck?: string, candidateName?: string): RegistrarVerificationResult => {
      ...
      if (match) {
        ...
        return {
          ...
          record: match,
          message: `Academic Record Confirmed! Verified graduate: ${match.fullName} (${match.course}, Class of ${match.batchYear}).`
        };
      }
